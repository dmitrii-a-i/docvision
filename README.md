...
> **Важно:** текущий API использует generic промпт (без примера JSON для типа документа).
> Метрики few_shot ниже получены в offline-эксперименте, где модель получала пример
> ожидаемого JSON — это верхняя граница качества, достижимая при интеграции few_shot
> в продакшн-промпт.
